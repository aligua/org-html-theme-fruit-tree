...
$(document).ready(function() {
    // Assuming that the ZeroClipboard swf file is in the same folder than ft,
    // get the path to it (it will be relative to the current page location).
    var bbScriptPath = $('script[src$="ft.js"]').attr('src');  // the js file path
    var bbPathToZeroClipboardSwf = bbScriptPath.replace('ft.js', 'ZeroClipboard.swf');
    ...
    $(document).on('mouseenter', '.org-src-container', function () {
        ...
        if ((window.location.protocol != 'file:') && ($(this).find('.zclip').length == 0)) {
            $(this).find('.copy-to-clipboard-button').zclip({
                ...
                //path: 'styles/ft/js/ZeroClipboard.swf',
